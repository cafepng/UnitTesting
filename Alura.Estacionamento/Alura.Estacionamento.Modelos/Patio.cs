...
namespace Alura.Estacionamento.Modelos
{
    public class Patio
    {
        ...
        {
            Veiculo result = this.Veiculos.Where(x => x.IdTicket == idTicket).FirstOrDefault();

            return result;
        }

        public Veiculo AlteraDadosVeiculo(Veiculo v)
        {
            Veiculo veiculo = this.Veiculos.Where(x => x.Placa == v.Placa).FirstOrDefault();

            veiculo.AlterarDados(v);
